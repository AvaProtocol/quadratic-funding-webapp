...
const LineCharts = (props) => {
  ...
  useEffect(async () => {
    ...
    const beginDay = Math.floor(moment.duration(projectVotes[0].timestamp).asDays());
    ...
    const labels = _.map(dayDatas, (item) => {
      return moment(projectVotes[0].timestamp).add(item.day, 'day').format('YYYY/M/D');
    });
  ...
  return (
    <LineChartsStyle {...props}>
      {!_.isEmpty(chartData) && (<Line height={370} data={chartData} options={{ maintainAspectRatio: false }}  />)}
    </LineChartsStyle>
  );
};
